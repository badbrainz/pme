...
{    
  ...
  for(int i=0;i<6;i++)
  { 
    magnitude = fastSquareRoot(frustumPlane[i].x * frustumPlane[i].x + 
...
const bool Frustum::pointInFrustum(const Tuple3f & center)
{
...
const float Frustum::getDistanceToPlane(int i, const Tuple3f &center){
...
const bool Frustum::sphereInFrustum(const Tuple3f &center, float radius)
{
...
{
  Tuple3f  center   = aabb.getCenterAABB(),
