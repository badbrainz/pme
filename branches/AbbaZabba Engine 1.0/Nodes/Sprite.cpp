...
bool Sprite::loadXMLSettings(XMLElement *sprite)
{
  ...
  if(child1 = sprite->getChildByName("description"))
    return IOXMLObject::loadXMLSettings(child1->getValuec());
...
void     Sprite::setRange(float rangeArg){ range = rangeArg; }
float    Sprite::getRange()              { return range;     }

void     Sprite::setTexture(const char * textureName){ texture.load2D(textureName); }
void     Sprite::setTexture(Texture &tex)            { texture = tex;               }
const    Texture &Sprite::getTexture() const            { return texture;              }

void     Sprite::setColor(float r, float g, float b, float a){ color.set(r,g,b,a);}
void     Sprite::setColor(const Tuple4f &clr)                { color = clr;       }
const    Tuple4f &Sprite::getColor() const                      { return color;      }

void     Sprite::setSize(float s){ size = (s <= 0.0) ? size : s; }
float    Sprite::getSize()       { return size;                  }

void           Sprite::setPosition(float x, float y, float z){ position.set(x,y,z);}
void           Sprite::setPosition(const Tuple3f &pos)       { position = pos;     }
const Tuple3f &Sprite::getPosition() const             { return position;    }

const Spline  &Sprite::getSpline() const { return path; }
