...
void Camera::rotateView(float angle, float x, float y, float z)
{
  ...
  float cosTheta = fastCos(angle),
        sinTheta = fastSin(angle);
...
void Camera::setKeyboardInput(KeyEvent &event, bool active)
{
  ...
  {
    case KEY_W:
    case KEY_UP:    activeKeys[0] = active; break;
    case KEY_S:
    case KEY_DOWN:  activeKeys[1] = active; break;
    case KEY_A:
    case KEY_LEFT:  activeKeys[2] = active; break;
    case KEY_D:
    case KEY_RIGHT: activeKeys[3] = active; break;
    case KEY_E:     activeKeys[4] = active; break;
    case KEY_Q:     activeKeys[5] = active; break;
    default: break;
  }
}

void Camera::setViewerPosition(const Tuple3f &position_){ position = position_; }
void Camera::setFocusPosition (const Tuple3f &focus_)   { focus    = focus_;    }
void Camera::setUpDirection   (const Tuple3f &up_)      { up       = up_;       }

const Matrix4f &Camera::getModelViewMatrix()const { return viewMatrix; }
const Tuple3f  &Camera::getViewerPosition() const { return position;   }
const Tuple3f  &Camera::getFocusPosition()  const { return focus;      }
const Tuple3f  &Camera::getUpDirection()    const { return up;         }
