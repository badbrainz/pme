...
bool CritterLabScene::initialize()
{
  Scene::initialize();
  ...
  offscreenTexture.create2DShell("CritterOffscreenTexture", resolution, resolution, GL_RGBA8, GL_RGBA, GL_CLAMP_TO_EDGE, GL_CLAMP_TO_EDGE);
  currentInfo.texture.create2DShell("CritterCurrentTexture", 256, 256, GL_RGBA8, GL_RGBA, GL_CLAMP_TO_EDGE, GL_CLAMP_TO_EDGE);
  ...
  if (btnPanel)
    for (int i = 0; i < 14; i++)
    {
      ...
      buttonTextures[i].create2DShell(String("CRL") + btnName, resolution, resolution, GL_RGB8, GL_RGB, GL_CLAMP_TO_EDGE, GL_CLAMP_TO_EDGE);
  ...
  const Array <ICString>* namelist = Gateway::getCritterDatabase()->getNamesList();
...
void CritterLabScene::beginScene()
{
  Scene::beginScene();
...
void CritterLabScene::actionPerformed(GUIEvent &evt)
{
    ...
    if (button->isClicked())
    {
      ...
      if (callbackString == "AcceptButton")
      {
        if (TransformGroup* group = rootGroup->getGroup(currentInfo.name))
        {
          ...
          Gateway::setActiveCritterElement(currentInfo);
          Gateway::setActiveCritter(rgroup);
...
void CritterLabScene::update(const FrameInfo &frameInfo)
{
...
void CritterLabScene::orbitCamera(float tick)
{
  angle += 0.5f * tick;
...
void CritterLabScene::handleKeyEvent(KeyEvent evt, int extraInfo)
{
  Scene::handleKeyEvent(evt, extraInfo);
}

void CritterLabScene::previousGroup()
{
...
void CritterLabScene::nextGroup()
{
  int i, j, end;
  startIndex = clamp(startIndex + capacity, 0, (int) elementInfoList.length());
...
void CritterLabScene::firstGroup()
{
  int i, j, end;
  startIndex = 0;
  endIndex = clamp(startIndex + capacity, 0, (int) elementInfoList.length());
...
void CritterLabScene::lastGroup()
{
  int i, j, end;
...
void CritterLabScene::updateLabels()
{
  if (currentSetLabel)
    currentSetLabel->setLabelString(String("Models ") + endIndex + String("/") + int(elementInfoList.length()));
...
void CritterLabScene::flushUnusedGroups()
{
  PrototypeInfo* info;
...
void CritterLabScene::drawFullScreenImage(const GSElementInfo& info)
{
  Tuple3f cen;
  Tuple4i viewport;
  ...
  proto = Gateway::getCritterPrototype(info.name);
  ...
  //group->getTransform().rotateY(45.0f * DEG2RAD);
...
void CritterLabScene::updateButtonTextures(unsigned int st, unsigned int en)
{
...
void CritterLabScene::updateOffscreenTexture(Texture *targetTexture)
{
  Tuple4i viewport;
  glGetIntegerv(GL_VIEWPORT, viewport);
  glViewport(0, 0, resolution, resolution);
...
void CritterLabScene::drawFullScreenQuad(int width, int height)
{
...
GSElementInfo* CritterLabScene::getGSElementInfo(const char* name)
{
...
CritterLabScene::~CritterLabScene()
{
