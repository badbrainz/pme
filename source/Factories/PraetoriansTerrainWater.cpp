...
bool PraetoriansTerrainWater::loadPackedMedia(const char* path)
{
	...
	unsigned int signature, chunklength, texturescount, watercount, vertexcount, indexcount;
	...
	for (unsigned int i = 0; i < watercount; i++)
	{
		...
		{
			colors   = (Tuple4ub*) Gateway::aquireGeometryMemory(vertexcount * 4);
			vertices = (Tuple3f*)  Gateway::aquireGeometryMemory(vertexcount * 12);
			txcoords = (Tuple2f*)  Gateway::aquireGeometryMemory(vertexcount * 8);
		}
