...
CharacterLabScene::CharacterLabScene(const char* name) : Scene(name)
{
  angle = 0.0f;
  resolution = 128;
  startIndex = 0;
...
bool CharacterLabScene::initialize()
{
  Scene::initialize();
  ...
  offscreenTexture.create2DShell("CharacterOffscreenTexture", resolution, resolution, GL_RGBA8, GL_RGBA, GL_CLAMP_TO_EDGE, GL_CLAMP_TO_EDGE);
  currentInfo.texture.create2DShell("CharacterCurrentTexture", 256, 256, GL_RGBA8, GL_RGBA, GL_CLAMP_TO_EDGE, GL_CLAMP_TO_EDGE);
  ...
  if (btnPanel)
    for (int i = 0; i < 14; i++)
    {
      ...
      buttonTextures[i].create2DShell(String("CL") + btnName, resolution, resolution, GL_RGB8, GL_RGB, GL_CLAMP_TO_EDGE, GL_CLAMP_TO_EDGE);
  ...
  const Array <ICString>* namelist = Gateway::getCharacterDatabase()->getNamesList();
...
void CharacterLabScene::beginScene()
{
  Scene::beginScene();
...
void CharacterLabScene::endScene()
{
}

void CharacterLabScene::actionPerformed(GUIEvent &evt)
{
    ...
    if (button->isClicked())
    {
      ...
      if (callbackString == "AcceptButton")
      {
        if (TransformGroup* group = rootGroup->getGroup(currentInfo.name))
        {
          ...
          Gateway::setActiveCharacterElement(currentInfo);
          Gateway::setActiveCharacter(rgroup);
...
void CharacterLabScene::update(const FrameInfo &frameInfo)
{
...
void CharacterLabScene::orbitCamera(float tick)
{
  angle += 0.5f * tick;
...
void CharacterLabScene::handleKeyEvent(KeyEvent evt, int extraInfo)
{
  Scene::handleKeyEvent(evt, extraInfo);
}

void CharacterLabScene::previousGroup()
{
...
void CharacterLabScene::nextGroup()
{
  int i, j, end;
  startIndex = clamp(startIndex + capacity, 0, (int) elementInfoList.length());
...
void CharacterLabScene::firstGroup()
{
  int i, j, end;
  startIndex = 0;
  endIndex = clamp(startIndex + capacity, 0, (int) elementInfoList.length());
...
void CharacterLabScene::lastGroup()
{
  int i, j, end;
...
void CharacterLabScene::updateLabels()
{
  if (currentSetLabel)
    currentSetLabel->setLabelString(String("Models ") + endIndex + String("/") + int(elementInfoList.length()));
...
void CharacterLabScene::flushUnusedGroups()
{
  PrototypeInfo* info;
...
void CharacterLabScene::drawFullScreenImage(const GSElementInfo& info)
{
  Tuple3f cen;
  Tuple4i viewport;
  ...
  proto = Gateway::getCharacterPrototype(info.name);
...
void CharacterLabScene::updateButtonTextures(unsigned int st, unsigned int en)
{
...
void CharacterLabScene::updateOffscreenTexture(Texture *targetTexture)
{
  Tuple4i viewport;
  glGetIntegerv(GL_VIEWPORT, viewport);
  glViewport(0, 0, resolution, resolution);
...
void CharacterLabScene::drawFullScreenQuad(int width, int height)
{
...
GSElementInfo* CharacterLabScene::getGSElementInfo(const char* name)
{
...
CharacterLabScene::~CharacterLabScene()
{
